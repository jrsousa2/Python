...
Log_artist = "D:\\Python\\Log_Artist.txt"
Log_title = "D:\\Python\\Log_Title.txt"
Log_year = "D:\\Python\\Log_Year.txt"

#Log_year = "D:\\Python\\Log_Year.txt"
Excel_Art = "D:\\Python\\Excel_Artist.xlsx"
Excel_Title = "D:\\Python\\Excel_Title.xlsx"
Excel_Year = "D:\\Python\\Excel_Year.xlsx"
...
#File: @Alceu Valença - Solidão@ Hits: @18@ Master Count @16@ Year @1984@ Release Count @2@ Year @1984@
#API: search: 1 Title: 1 songlist: 7 Ways To Love
#Art_layout = ["API", "Search", "Cur_Artist", "Srch_Artists", "Srch_Variations"]
...
# OPENS AN EXCEL FILE (IF IT DOESN"T EXIST, CREATE)
def open_excel(Arq,layout):
    dict = {}
    dict["file"] = None
    dict["sheet"] = None
    if Excel:
        if not isfile(Arq):
            ...
            for i in range(len(layout)):
                worksheet.cell(row=1, column=i+1, value=layout[i])
        else:
            workbook = openpyxl.load_workbook(Arq)
...
def write_Art(worksheet,next_row,API,search,Art,Srch_art,Srch_varis):
    nrow = next_row
    nbr_arts = len(Srch_art)
    if nbr_arts==0:
       Srch_art.append("") 
       Srch_varis.append([])
    for i in range(nbr_arts):
        worksheet.cell(row=nrow, column=1, value=API)
        worksheet.cell(row=nrow, column=2, value=search)
        worksheet.cell(row=nrow, column=3, value=Art)
        worksheet.cell(row=nrow, column=4, value=len(Srch_art))
        worksheet.cell(row=nrow, column=5, value=Srch_art[i])
        if not i<len(Srch_varis):
           Srch_varis.append([]) 
        worksheet.cell(row=nrow, column=6, value=list_to_str(Srch_varis[i]))
        nrow = nrow+1
    return nrow        

# WRITE TO TITLE EXCEL
def write_Title(worksheet,next_row,API,search,Title,songlist):
    nrow = next_row
    nbr_songs = len(songlist)
    if nbr_songs==0:
       songlist.append("None") 
    for i in range(nbr_songs):
        worksheet.cell(row=nrow, column=1, value=API)
        worksheet.cell(row=nrow, column=2, value=search)
        worksheet.cell(row=nrow, column=3, value=Title)
        worksheet.cell(row=nrow, column=4, value=len(songlist))
        worksheet.cell(row=nrow, column=5, value=songlist[i])
        nrow = nrow+1
    return nrow    

# WRITE TO TITLE EXCEL
def write_Year(worksheet,next_row,Art,Title,Hits,Mast_cnt, Mast_Year, Rel_cnt, Rel_Year):
    nrow = next_row
    worksheet.cell(row=nrow, column=1, value=Art)
    worksheet.cell(row=nrow, column=2, value=Title)
    worksheet.cell(row=nrow, column=3, value=Hits)
    worksheet.cell(row=nrow, column=4, value=Mast_cnt)
    worksheet.cell(row=nrow, column=5, value=Mast_Year)
    worksheet.cell(row=nrow, column=6, value=Rel_cnt)
    worksheet.cell(row=nrow, column=7, value=Rel_Year)
    nrow = nrow+1
    return nrow
...
def Call_art_match(srch_res,Obj,k,j,Art,excelf,next_row):
    ...
    # PRINT RESULTS FOR NO MATCH write_Art(worksheet,next_row,API,search,Art,Srch_art,Srch_var):
    if Excel and Has_art_list and not Art_match:
       next_row = write_Art(excelf["sheet"],next_row,k+1,j+1,Art,Names_list,Variations_list)
    dict = {}
    dict["res"] = Art_match   
    dict["next_row"] = next_row
    return dict 

# THE BELOW IS A LIST OF CLASS OBJECTS
def Call_title_match(Obj,k,j,Art,Title,excelf,next_row):
    dict = {}
    ...
    if Excel and Has_tracklist and not Title_match: 
       next_row = write_Title(excelf["sheet"],next_row,k+1,j+1,Title,songlist)
...
def Call_disco(PL_name=None,PL_nbr=None):
   ...
   #  read_PL = PLs.ItemByName(PL_main_nm)
   #  # LINHA QUE PREPARA A PL
   ...
   #  print("Doublecheck playlist:",read_PL.Name,"\n")
    ...
    # CAPTURE DATA
    excelf = {}
    excelf["Art"] = open_excel(Excel_Art,Art_layout)
    excelf["Title"] = open_excel(Excel_Title,Title_layout)
    ...
    while (Online and i<nbr_files):
        ...
        if file_exists and Arq[i]==Curr_loc and Online and Files.Is_DMP3(Arq[i]):
           ...
           while (k<nbr_calls and elapsed_time<time_lim_sec):
                  ...
                  while(j<nbr_matches and fixed["Searches"]<20 and elapsed_time<time_lim_sec):
                        ...
                        if not Skip and type_ID is not None: 
                           ...
                           # CALL ART MATCH
                           Art_match_dic = Call_art_match(srch_res,Obj,k,j,Art[i],excelf["Art"],next_row["Art"])
                           Art_match = Art_match_dic["res"]
                           next_row["Art"] = Art_match_dic["next_row"]
                           # CALL TITLE MATCH
                           Title_match_dic = Call_title_match(Obj,k,j,Art[i],Title[i],excelf["Title"],next_row["Title"])
                           Title_match = Title_match_dic["res"]
                           next_row["Title"] = Title_match_dic["next_row"]
           ...
           if Excel:
              ...
              excelf["Title"]["file"].save(Excel_Title) 
           ...
           if tam>0:
              ...
              if Excel:
                 next_row["Year"] = write_Year(excelf["Year"]["sheet"],next_row["Year"],Art[i],Title[i],tam,Mast_cnt, Mast_Year, Rel_cnt, Rel_Year)
                 excelf["Year"]["file"].save(Excel_Year)   
              # PICK THE TRACK
