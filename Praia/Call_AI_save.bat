...
REM MODEL PROTEUS
set Model="tvai_up=model=prob-4:scale=0:w=1280:h=720:preblur=0:noise=0:details=0:halo=0:blur=0:compression=0:estimate=8:blend=0.2:device=0:vram=0.1:instances=1,scale=w=1280:h=720:flags=lanczos:threads=0:force_original_aspect_ratio=%crop%"
...
if %frames_to_go% lss %total_frames% (
    ffmpeg -hide_banner -start_number %frames_to_go% -i "%input_dir%\frame_%%04d.png"  "-sws_flags" "spline+accurate_rnd+full_chroma_int" "-filter_complex" %Model% "-c:v" "png" "-pix_fmt" "rgb24" -start_number %frames_to_go% "%output_dir%\frame_%%04d.png"
) else (
