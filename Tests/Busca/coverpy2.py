...
class Result:
	...
	def parse(self, item):
		...
		#except KeyError as e:
			...
			#raise Call_art.exceptions.NoResultsException
...
class CoverPy: 
	...
	def get_cover(self, term, dict, limit = 1, debug = False):
		# Get an album cover, return a Result object 
		search = self._search(term, limit)

		# Preciso evitar exceptions aqui tb
		try:
			parsed = search.json()
		except Exception:
			print("Json exception")
			dict.append({'Json exc':1})
			pass
		#AttributeError:
		#			ss=1
		else:
			...
			if parsed['resultCount']>0:
				dict[0]['Json exc'] = 0
				#dict[0].append({'Json exc':0})
			else:
				dict.append({'Json exc':0})
