...
For Each Arg in Args 
  ...
  If Instr(1,arg,"-",1) = 1 or Instr(1,arg,"/", 1) = 1 Then
    ...
    If UCase(arg) = "-S" or UCase(arg) = "/S" then
       Sort_Fields = True
    End If
    If UCase(arg) = "-C" or UCase(arg) = "/C" then
       Count_Fields = True
    End If   
    If UCase(arg) = "-X" or UCase(arg) = "/X" then
       Date_Fields = True
    End If
  End If
Next
...
If Art_Fields Then
   ...
   objExcel.Cells(1, 12) = "Cover_Mime_Type"
   Marca = Marca+2
End If
   
If Hex_Fields Then 
   Marca_Hex = Marca  
   objExcel.Cells(1, Marca_Hex+1) = "Art_Size2"
...
If Dim_Fields Then
   ...
   objExcel.Cells(1, Marca_Dim+3) = "Art_Size"
   Marca = Marca+3
End If

If Sort_Fields Then
   Marca_Sort = Marca
   objExcel.Cells(1, Marca_Sort+1) = "Grouping"
   objExcel.Cells(1, Marca_Sort+2) = "Art_Sort"
   objExcel.Cells(1, Marca_Sort+3) = "AA_Sort"
   Marca = Marca+3
End If

If Count_Fields Then
   Marca_Count = Marca
   objExcel.Cells(1, Marca_Count+1) = "PlayedCount"
   objExcel.Cells(1, Marca_Count+2) = "SkippedCount"
   Marca = Marca+2
End If

If Date_Fields Then
   Marca_Date = Marca
   objExcel.Cells(1, Marca_Date+1) = "Date_added"
End If

Set iTunesApp  = CreateObject("iTunes.Application.1")
Set sources = iTunesApp.Sources

If Art_Fields Or Dim_Fields Then On Error Resume Next
		...
		'result = 41
 		   	...
 		   	For m = 1 to numtracks
                  ...
                  End If   
                     ...
                     If Formato="jpg" Or Formato="png" Then
                        CoverSize track.Location, Formato, Art_Size2, Hex_Size, strHex
                     End If
                   End If                      
         ...
         'CHECK YEAR          
 		   ...
 		   'COLUNAS=ARTIST,ALBUM ARTIST,NAME,ALBUM,YEAR,LOCATION,HASART
                   ...
                   '3a col. eh chamada Bytes (subset de chars da cover)
                   If Hex_Fields Then
                      objExcel.Cells(NumRows, Marca_Hex+1) = Art_Size2
                   ...
                   If Dim_Fields Then
                      ...
                      objExcel.Cells(NumRows, Marca_Dim+3) = Art_Size
                   End If
                   ...
                   If Count_Fields Then
                      objExcel.Cells(NumRows, Marca_Count+1) = track.PlayedCount
                      objExcel.Cells(NumRows, Marca_Count+2) = track.Skippedcount
                      'objExcel.Cells(NumRows, 17) = track.grouping
                   End If

                  'LAST FIELD
                   If Date_Fields Then
                      objExcel.Cells(NumRows, Marca_Date+1) = track.dateadded
                   End If
